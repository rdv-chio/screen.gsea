...
#Compute observed enrichment scores for each gene set
	...
	for (i in 1:geneset.N) {
		...
		GSEA.results <- GSEA.EnrichmentScore_lite(gene.list=dataset.indexes, gene.set=geneset.indexes, weighted.score.type, correl.vector=dataset.effects)
...
# Compute enrichment scores for random permutations
	...
	for (i in 1:geneset.N) {
		...
		for (r in 1:nperm) {
			...
			GSEA.results <- GSEA.EnrichmentScore_lite(gene.list=reshuffled.dataset.indexes, gene.set=geneset.indexes, weighted.score.type, correl.vector=dataset.effects)   
...
# Produce report and running enrichment plot for each gene set passing the FDR q-value cut-off
	...
	for (i in 1:geneset.N) {
		...
		# Produce report
			...
			names(gene.report) <- c("#", "GENE", "LIST LOCATION", "MEAN EFFECT", "RES", "CORE_ENRICHMENT")
			gene.report <- gene.report[gene.report[[2]] != '',]
			if (output.directory != "")  {
				filename <- paste(output.directory, doc.string, ".", substring(geneset.names.2[i],1,20), ".report", ".", loc, ".txt", sep="", collapse="")
				write.table(gene.report, file = filename, quote=F, row.names=F, sep = "\t")
				if (.Platform$OS.type == "unix") {
					gs.filename <- paste(output.directory, doc.string, ".",substring(geneset.names.2[i],1,20), ".plot", ".", loc, ".pdf", sep="", collapse="")
