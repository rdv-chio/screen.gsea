...
#Compute observed enrichment scores for each gene set
	...
	for (i in 1:geneset.N) {
		...
		GSEA.results <- GSEA.EnrichmentScore_lite_3(gene.list=dataset.indexes, gene.set=geneset.indexes, weighted.score.type, correl.vector=dataset.effects)
...
# Compute enrichment scores for random permutations
	...
	for (i in 1:geneset.N) {
		...
		for (r in 1:nperm) {
			...
			GSEA.results <- GSEA.EnrichmentScore_lite_3(gene.list=reshuffled.dataset.indexes, gene.set=geneset.indexes, weighted.score.type, correl.vector=dataset.effects)   
